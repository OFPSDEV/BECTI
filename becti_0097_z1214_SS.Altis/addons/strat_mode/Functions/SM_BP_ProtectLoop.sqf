...
while {!CTI_GameOver} do {
	if (({(_x getVariable "cti_town_sideID") == _side_id } count _neigh) == count _neigh) then {
		...
		{ if (!(_x in _protected) && ((_x distance _base) <= CTI_BASE_AREA_RANGE)) then { diag_log format [ "%1 :: Protecting %2" , _base,_x]; _x allowDammage false; _protected set [count _protected,_x];};true } count _objects;
	} else {
		{diag_log format [ "%1 :: Protecting off  %2" , _base,_x];_x allowDammage true;true} count _protected;
		_protected = [];
	};
	sleep 10;
};
