...
while { true } do {
	...
	if ((!alive player) || (!alive CTI_ConstructionCam_HQ) || (!(call CTI_CL_FNC_IsPlayerCommander)) || (((getPos CTI_ConstructionCam_HQ) distance CTI_ConstructionCamera) > (CTI_BASE_CONSTRUCTION_RANGE+5)) ) exitWith {closeDialog 0; CTI_ConstructionCam_BuildingID = -1;}; 
