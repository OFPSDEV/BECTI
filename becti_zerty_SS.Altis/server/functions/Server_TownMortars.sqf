...
CTI_SE_FNC_MortarGetShellPosition = {
		...
		if !(_x in _targets_scanned) then {
			...
			if ((_side countSide _near) == 0) then {
				if (random 100 <= _precognition || _defensive_group knowsAbout _x >= 1) then {
					[_targets, _near] Call CTI_CO_FNC_ArrayPush;
					{if !(_x in _targets_scanned) then {[_targets_scanned, _x] Call CTI_CO_FNC_ArrayPush}} forEach _near;
				};
			};
		};
	...
	_targets_force = [];
	for '_i' from 0 to (count _targets)-1 do {{[_targets_force, _i] Call CTI_CO_FNC_ArrayPush} forEach (_targets select _i)}; //--- The more the targets, the more the chance to target.
...
CTI_SE_FNC_SpawnTownMortars = {
	...
	_set = [];
	if ((_town getVariable "cti_town_value") >= 400) then {
	...
	_mortars = [[],[]]; //--- First is mortar, second are statics (walls..).
	{
		...
		[_mortars select _index, _created] Call CTI_CO_FNC_ArrayPush;
	} forEach _set;
