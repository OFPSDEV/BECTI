...
Since the AI in ArmA III can be realy frustrating while driving, the ability of remote controling the drivers have been introduced.<br/>
You can initiate the remote controling using the action menu of the unit. (F1-10 Keys and then the 6 key). <br />
...
_marker setMarkerTextLocal format ["Name: Warfare CTI - Seattle , WA www. OFPS .net"];
_pos=[(_pos select 0),(_pos select 1)-_offset,0];
...
_marker setMarkerColorLocal "ColorWhite";
_marker setMarkerAlphaLocal 0.8;
_marker setMarkerTextLocal format ["Misson Name: becti_zerty121_SS"];
_pos=[(_pos select 0),(_pos select 1)-_offset,0];
...
_marker setMarkerColorLocal "ColorWhite";
_marker setMarkerAlphaLocal 0.6;
_marker setMarkerTextLocal format ["2. No Friendly Asset Destruction (Vehicles, mortar tubes, defense structures, etc)"];
_pos=[(_pos select 0),(_pos select 1)-_offset,0];
...
_marker setMarkerColorLocal "ColorWhite";
_marker setMarkerAlphaLocal 0.6;
_marker setMarkerTextLocal format ["5. DO NOT fill the commander slot if you are inexperienced (empty/light server is OK)"];
_pos=[(_pos select 0),(_pos select 1)-_offset,0];
