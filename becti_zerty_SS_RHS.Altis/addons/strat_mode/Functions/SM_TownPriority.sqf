...
while {! CTI_GameOver } do {
	...
	player addAction ["<t color='#ff9900'>SM# Set Active Town</t>",SM_Ask_Priority,[], 92, false, true,'', '!CTI_P_PreBuilding && alive _this  && ! CTI_P_Repairing && CTI_CL_FNC_IsPlayerCommander&& _this == player '];
	player addAction ["<t color='#ff9900'>SM# Force town Disactivation</t>",SM_Disactivate_Town,[], 92, false, true,'', '!CTI_P_PreBuilding && alive _this  && ! CTI_P_Repairing && CTI_CL_FNC_IsPlayerCommander && !(isNull ((CTI_P_SideJoined call CTI_CO_FNC_GetSideLogic) getVariable "CTI_PRIORITY") ) && time > (SM_Last_dis +SM_TO_dis)&& _this == player '];
