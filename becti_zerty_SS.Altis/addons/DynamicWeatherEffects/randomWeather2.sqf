...
weatherTemplates = [
        ...
        ["Overcast",[0,1,2,5],[0.50,0,0,2,2]],
        ["Light Rain",[1,1,3,5],[0.60,0.3,0.05,3,3]],
        ["Medium Rain",[2,2,4],[0.70,0.5,0.05,4,4]],
        ["Rainstorm",[3],[1,1,0.1,5,5]],
        ["Light Fog",[0,0,1,1,2,6],[0.4,0,[0.1,0.001,10],0,0]],
        ["Medium Fog",[5,5,7],[0.4,0,[0.2,0.01,30],0,0]],
        ["Dense Fog",[6],[0.5,0,[0.4,0.05,50],0,0]]
	];
...
// Check if there is no ParamsArray, and pick random if so, otherwise pick from paramsArray.
   ...
   // Start recurring weather loop.
    while {true} do {
        ...
        _n_time= abs((((weatherTemplates select rw2_Next_Weather) select 2) select 1)-(((weatherTemplates select rw2_Current_Weather) select 2) select 1))*6000;
        if (_n_time == 0) then {_n_time = 600};
